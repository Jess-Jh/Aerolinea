...
import co.edu.uniquindio.aerolinea.pilaBicolaListaEnlazada.Pila;
...
public class AerolineaController implements Initializable {
    ...
    @FXML
    private ImageView imgCarroEmbarqueR = new ImageView();
    ...
    ArrayList<PosicionCarro> posicionesCarrosPila = new ArrayList<>();
	...
	public void setAplicacion(AplicacionAerolinea aplicacionAerolinea) {
    	...
    	posicionesCarrosPila = inicializarPosicionesPila();
    	listaImagenes = llenarListaImagenes();
    	imgCarroEmbarqueR.setVisible(false);
    }
    ...
    void pasarEquipajeAlCarroEmbarque(MouseEvent event) {
    	...
    	if(listadoEquipajes.isEmpty()) {
    		...
    		if(equipajeSeleccionEmbarque != null) {
    			try {
					validarAvion(equipajeSeleccionEmbarque.getNumAvion());
					...
					aplicacionAerolinea.mostrarMensaje("Notificación Embarque Equipaje", "Notificación Embarque Equipaje", e.getMessage(),AlertType.WARNING);					    	
				}
    		}
    	}
    }
	...
	private void pesoTotalEquipajes() {
		double total = 0;
		if(!(listadoEquipajesEmbarque.isEmpty())) {
			for (Equipaje equipaje : listadoEquipajesEmbarque) {
				total += equipaje.getPesoTotal();
			}
			lblPesoTotalCarroEmbarque.setText(String.valueOf(total));
		} else {
			lblPesoTotalCarroEmbarque.setText(String.valueOf(total));
		}
	}
	...
	/**
	 * Validar que los equipajes vayan para el mismo vuelo
	 * @param numAvion
	 * @throws EquipajeException 
	 */
    private void validarAvion(String numAvion) throws EquipajeException {
    	if(!(listadoEquipajesEmbarque.isEmpty())) {
    		String equipajeLista = "";
    		
			for (Equipaje equipaje : listadoEquipajesEmbarque) {
				equipajeLista = equipaje.getNumAvion();
				break;
			}
			if(equipajeLista.equalsIgnoreCase(numAvion)) return;
			else throw new EquipajeException("Los equipajes deben tener el mismo Id del avión");
		}
	}
  ...
  //------------- Datos Embarque -------------------------->>
    ...
    Pila<CarroEmbarque> pilaCarros = new Pila<>();
    
    Bicola<ImageView> bicolaImagenesCarros = new Bicola<>();
    Pila<ImageView> pilaImagenesCarros = new Pila<>();
    	...
    	if(bicolaCarros.getTamaño() >= 10) {
    		...
    		for (int i = 0; i < listaImagenes.length; i++) {
    			...
    			if(!(listaImagenes[i].isDisable())) {
    				...
    				bicolaImagenesCarros.insertar(ubicarCarro);
    				break;
    			}
    		}
    ...
    /**
     * Volver a cambiar el color del botón cuando el carro ya haya sido retirado
     */
    ...
    void salidaCarroEmbarque(ActionEvent event) {
    	...
    	if(bicolaCarros.getTamaño() <= 0) {
			...
			try {
				...
				if(lblPesoTotalCarroEmbarque == null || lblPesoTotalCarroEmbarque.getText().equals("")) {
					aplicacionAerolinea.mostrarMensaje("Notificación Embarque Equipaje", "Notificación Embarque Equipaje", "Seleccione equipajes para el carro embarque", AlertType.WARNING);		
					
				} else {
					pesoTotal = validarPeso(lblPesoTotalCarroEmbarque);
					CarroEmbarque carroSalida = bicolaCarros.desencolar();
					
					ArrayList<Equipaje> listaEquipajesEmbarque = new ArrayList<>();
					listaEquipajesEmbarque.addAll(listadoEquipajesEmbarque);
					carroSalida.setPeso(pesoTotal);
					carroSalida.setListaEquipajes(listaEquipajesEmbarque);
					...
					aplicacionAerolinea.mostrarMensaje("Notificación Embarque Equipaje", "Notificación Embarque Equipaje", "Ha salido el carro embarque " + carroSalida.getNumIdentificacion() + " hacia el vuelo " + carroSalida.getNumAvion(), AlertType.INFORMATION);		
				}
	
			} catch (EquipajeException e1) {
				aplicacionAerolinea.mostrarMensaje("Notificación Embarque Equipaje", "Notificación Embarque Equipaje", e1.getMessage(), AlertType.WARNING);		
			}
    ...
    /**
     * Animación de la salida de un carro embarque 
     */
    private void animacionSalidaCarroEmbarque() {
    				
		ImageView ubicarCarro = bicolaImagenesCarros.desencolar();	
		...
		transition.setOnFinished(new EventHandler<ActionEvent>() {
		    ...
		    public void handle(ActionEvent event) {
		    	...
		    	Nodo<ImageView> auxCarro = bicolaImagenesCarros.getPrimero();
				for (int j = 0; j < bicolaImagenesCarros.longitud(); j++) {    	
					ubicarCarro = auxCarro.getValorNodo();
					...
					if(auxCarro.getSiguienteNodo() != null) {
						auxCarro = auxCarro.getSiguienteNodo();
						ubicarCarro = auxCarro.getValorNodo();
					}
				}
				transition2.play();
	...
	/**
     * Validar el peso total del carro embarque
     * @param lblPesoTotalCarroEmbarque2
     * @return 
     * @throws EquipajeException 
     */
    private double validarPeso(Label lblPesoTotalCarroEmbarque2) throws EquipajeException {
    	...
    	if(pesoTotal <= 0) {
    		notificacion = "Seleccione los equpajes para el carro embarque";
    	} else if(pesoTotal > 500.0)
    		notificacion = "El peso de los equipajes excede el peso máximo para el carro embarque";
    	
    	if(notificacion.equals("")) return pesoTotal;
    	else throw new EquipajeException(notificacion);
	}
	...
	private void animacionRetirarCarroEmbarque(CarroEmbarque carroEncontrado, int tamañoPila) {
		...
		while(!bicolaImagenesCarros.isVacia()) {	
			Nodo<ImageView> auxCarro = bicolaImagenesCarros.getPrimero();
																	  
			if(auxCarro.getValorNodo().getId().equalsIgnoreCase(carroEncontrado.getNumIdentificacion())) {
				ImageView ubicarCarroEncontrado = bicolaImagenesCarros.desencolar();
									
				transition.setDuration(javafx.util.Duration.seconds(10));
				transition.setToX(1292);
				transition.setNode(ubicarCarroEncontrado);
				validarBoton();
				transition.play();
				
				transition2.setToX(-5);
				transition2.setNode(ubicarCarroEncontrado);
				
				transition.setOnFinished(new EventHandler<ActionEvent>() {
					
					@Override
					public void handle(ActionEvent event) {
						ubicarCarroEncontrado.setVisible(false);
						ubicarCarroEncontrado.setDisable(false); 
						transition2.play();
						
						Nodo<ImageView> auxCarrosQuedan = bicolaImagenesCarros.getPrimero();
						for (int l = 0; l < bicolaImagenesCarros.longitud(); l++) {    			    					
							ImageView ubicarCarro1 = auxCarrosQuedan.getValorNodo();
							...
							if(auxCarrosQuedan.getSiguienteNodo() != null) {
								auxCarrosQuedan = auxCarrosQuedan.getSiguienteNodo();
								ubicarCarro1 = auxCarrosQuedan.getValorNodo();
							}
						}
						
						transition3.setOnFinished(a -> {
							Nodo<ImageView> auxCarrosPila = pilaImagenesCarros.getCima();
							for (int j = 0; j < pilaImagenesCarros.getTamano(); j++) {    
								
								ImageView ubicarCarro2 = auxCarrosPila.getValorNodo();
								...
								if(auxCarrosPila.getSiguienteNodo() != null) {
									auxCarrosPila = auxCarrosPila.getSiguienteNodo();
									ubicarCarro2 = auxCarrosPila.getValorNodo();
								}
							}
						});
						transition4.setOnFinished(ex -> {
							
							while(!pilaImagenesCarros.estaVacia()) {
								ImageView ubicarCarro3 = pilaImagenesCarros.getCima().getValorNodo();
								bicolaImagenesCarros.ponerFinal(pilaImagenesCarros.pop());
				...
				colocarCarrosPila(bicolaImagenesCarros.desencolar());
			}
		}
	...
	/**
	 * Habilitar una posicion que queda libre
	 */
	private void habilitarPosicion(ImageView ubicarCarro1) {
		for (PosicionCarro posicion : posicionesCarros) {
			if(posicion.getPosicion() == ubicarCarro1.getTranslateX()) {
				posicion.setOcupado(false);
			}
		}
	}
     ...
     * @return
     */
	private ArrayList<PosicionCarro> inicializarPosicionesPila() {
		ArrayList<PosicionCarro> listaPosiciones = new ArrayList<>();
		
		PosicionCarro posicion1 = new PosicionCarro(568);
		PosicionCarro posicion2 = new PosicionCarro(512);
		PosicionCarro posicion3 = new PosicionCarro(457);
		PosicionCarro posicion4 = new PosicionCarro(402);
		PosicionCarro posicion5 = new PosicionCarro(347);
		PosicionCarro posicion6 = new PosicionCarro(292);
		PosicionCarro posicion7 = new PosicionCarro(237);
		PosicionCarro posicion8 = new PosicionCarro(182);
		PosicionCarro posicion9 = new PosicionCarro(127);
		PosicionCarro posicion10 = new PosicionCarro(72);
